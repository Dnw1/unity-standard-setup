...
namespace com.dnw.standardpackage
{
    ...
    private void ApplyVideoShapeAndStereo( string fileName, VRVideoPlayer vrVidPlayer, VideoSettings videoSettings) {
        if (vrVidPlayer == null || videoSettings == null) {
            Debug.LogWarning("[VideoManager]: Cannot apply video settings (missing references)");
            return;
        }

        // Extract logical video name (same logic you already use for volume)
        string videoName = ExtractVideoName(fileName);

        Videos videoConfig = null;
        if (data != null && data.videos != null) {
            videoConfig = Array.Find(data.videos, v => v.name == videoName);
        }

        float shapeValue = videoConfig?.videoException.HasValue == true
            ? videoConfig.videoException.Value
            : videoSettings.videoShape;

        VRVideoPlayer.VideoShape shape = shapeValue switch
        {
            180 => VRVideoPlayer.VideoShape._180,
            360 => VRVideoPlayer.VideoShape._360,
            _   => VRVideoPlayer.VideoShape._360
        };

        vrVidPlayer.SetShape(shape);

        // ---- STEREO ----
        string stereoValue = !string.IsNullOrEmpty(videoConfig?.stereoException)
            ? videoConfig.stereoException
            : videoSettings.videoStereo;

        VRVideoPlayer.VideoStereo stereo = stereoValue switch
        {
            "Mono"       => VRVideoPlayer.VideoStereo.Mono,
            "TopBottom"  => VRVideoPlayer.VideoStereo.TopBottom,
            "LeftRight"  => VRVideoPlayer.VideoStereo.LeftRight,
            _            => VRVideoPlayer.VideoStereo.Mono
        };

        vrVidPlayer.SetStereo(stereo);

        Debug.Log(
            $"[VideoManager]: Applied video settings → " +
            $"Shape: {shape} | Stereo: {stereo} " +
